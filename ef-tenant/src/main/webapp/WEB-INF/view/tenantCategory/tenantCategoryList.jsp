    ...
    <table class="am-table am-table-striped am-table-hover table-main">
      ...
      <c:forEach items="${objectList}" var="tenantProduct" end="4">
        <tr id="${tenantProduct.id}">
          <td>
            <a id="${tenantProduct.id}" class="am-btn am-btn-default am-btn-xs am-text-secondary" href="javascript:void(0);"  onclick="showConfirm('提示','确定移除该商品?。',function(){removeTenantProject('${tenantProduct.tenantCategory.id}','${tenantProduct.product.id}','${tenantProduct.id}')})"><span
...
<script>
  function removeTenantProject(tenantId,projectId,tenantProjectId){
    $.ajax({
      type: "get",
      url: '<c:url value="/basic/xmj.do?qm="/>',
      cache: false,
      dataType: "json",
