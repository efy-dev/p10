...
<head>
    <script>
        var gobackUrl = "<c:url value="/product/productModel/${purchaseOrder.purchaseOrderProductList.get(0).productModel.id}"/>"
    </script>
    <meta charset="utf-8">
...
<div id="cardDiv" class="gift ae">
    ...
    <div class="car-state ae" style="padding-bottom: 0px;">
        <div class="btb"><h5>礼物状态</h5><span id="giftStatus"><c:if test="${purchaseOrder.orderStatus=='6'}">未收礼</c:if>
            <c:if test="${purchaseOrder.orderStatus!='6'&&purchaseOrder.orderStatus!='1'}">已收礼</c:if></span></div>
        <c:if test="${purchaseOrder.orderStatus=='6'||purchaseOrder.orderStatus=='1'}">
        ...
        <div class="btn-again">
            <a href="<c:url value="/order/giftBuy/${purchaseOrder.purchaseOrderProductList.get(0).productModel.id}/${purchaseOrder.purchaseOrderProductList.get(0).purchaseAmount}"/>"
               title="再买一份">再买一份</a>
        </div>
        <div class="btn-again" style="margin-top: 0">
            <a href="<c:url value="/order/easyBuy/${purchaseOrder.purchaseOrderProductList.get(0).productModel.id}?amount=${purchaseOrder.purchaseOrderProductList.get(0).purchaseAmount}"/>" title="送给自己">送给自己</a>
        </div>
    ...
    <div class="car-fx" id="giftButton">
        ...
        <div class="car-fx" id="giftButton">
            ...
            <a href="<c:url value="/createGiftImage/${purchaseOrder.id}"/> ">保存为图片</a>
...
<script type="text/javascript">
    ...
    <c:if test="${purchaseOrder.orderStatus!='6' && purchaseOrder.orderStatus!='5'}">
    function checkOrderStatus() {
        ajaxRequest("<c:url value="/order/checkOrderStatus.do"/>", {"purchaseOrderId": "${purchaseOrder.id}"}, function (data) {
            if (data == "6") {
                $("#giftButton").show();
                $("#giftStatus").html("未收礼");
            } else {
