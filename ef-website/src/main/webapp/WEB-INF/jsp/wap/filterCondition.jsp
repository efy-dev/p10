    ...
    <div class="wh search">
        <div class="bd search-item">
            ...
            <ul id="classify" class="bd ul-list">
                <li <c:if test="${empty searchParamBean.queryFacet}">class="active"</c:if>><a href="javascript:void(0);"
                                                                                              title="全 部"
                                                                                              onclick="classify(this, '')">全
                    部</a></li>
                <c:forEach items="${searchParamBean.facetFieldsMap}" var="facetFields">
                    <c:forEach items="${facetFields.value}" var="facetEntry">
                        ...
                        <li
                                <c:if test="${not empty searchParamBean.queryFacet && fn:contains(searchParamBean.queryFacet, key)}">class="active"</c:if>>
                            <a href="javascript:void(0);"
                               onclick="classify(this, '${facetEntry.key}')">${facetEntry.key}</a>
                        </li>
                    </c:forEach>
                </c:forEach>
            </ul>
        ...
        <div class="bd search-item">
            ...
            <ul id="price" class="bd ul-list">
                <li
                        <c:if test="${empty searchParamBean.fq || searchParamBean.fq == 'product_model_price:[* TO *]'}">class="active"</c:if>>
                ...
                <li <c:if test="${searchParamBean.fq == 'product_model_price:[1000 TO 5000]'}">class="active"</c:if>>
                    <a href="javascript:void(0);" onclick="price(this, '[1000 TO 5000]')"
                       title="1000~5000">1000~5000</a>
                </li>
