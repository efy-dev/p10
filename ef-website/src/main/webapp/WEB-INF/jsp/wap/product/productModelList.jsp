...
<artice>
    <div class="bd search-sort">
        <ul class="bd tnav">
            <li><a href="<c:url value="/product/list/${project.id}?sort=desc:saleAmount&pageEntity.index=1"/>"
                   title="销 量">销 量</a></li>
            <li>
                <c:if test="${str==null||flag==2||flag==3||flag==4}">
                    <a href="<c:url value="/product/list/${project.id}?sort=desc:price&pageEntity.index=1"/>"
                       title="销 量">价 格</a>
                </c:if>
                <c:if test="${flag==1&&str=='desc'}">
                    <a href="<c:url value="/product/list/${project.id}?sort=asc:price&pageEntity.index=1"/>"
                       title="销 量">价 格</a><i class="icon-a2"></i>
                </c:if>
                <c:if test="${flag==1&&str=='asc'}">
                    <a href="<c:url value="/product/list/${project.id}?sort=desc:price&pageEntity.index=1"/>"
                       title="销 量">价 格</a><i class="icon-a1"></i>
                </c:if>
            </li>
            <li><a href="<c:url value="/product/list/${project.id}?sort=desc:popularityAmount&pageEntity.index=1"/>"
                   title="人 气">人 气</a></li>
            <li>
                <a href="<c:url value="/product/list/${project.id}?sort=desc:product.createDateTime&pageEntity.index=1"/>"
                   title="新 品">新 品</a></li>
        </ul>
                ...
                <c:forEach items="${productModelList}" var="productModel" varStatus="rec">
                    <li>
                        <a href="<c:url value="/product/productModel/${productModel.id}"/>" style="color: #000">
                            <img src="http://pro.efeiyi.com/${productModel.productModel_url}@!product-model-wap-view"
                                 alt="">
                            <c:set var="name">
