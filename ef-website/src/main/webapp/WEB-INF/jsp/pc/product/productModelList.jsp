            ...
            <li id="price">
                ...
                <c:if test="${flag==1&&str=='desc'}">
                    <a href="<c:url value="/product/list/${project.id}?sort=asc:price&pageEntity.index=1"/>"
                       title="价  格">价 格<i class="icon arrow-down"></i></a>
                </c:if>
                <c:if test="${flag==1&&str=='asc'}">
                    <a href="<c:url value="/product/list/${project.id}?sort=desc:price&pageEntity.index=1"/>"
                       title="价  格">价 格<i class="icon arrow-up"></i></a>
                </c:if>
            </li>
            <li id="popularityAmount"><a
                    href="<c:url value="/product/list/${project.id}?sort=desc:popularityAmount&pageEntity.index=1"/>"
                    title="人  气">人 气</a></li>
            <li id="saleAmount"><a
                    href="<c:url value="/product/list/${project.id}?sort=desc:saleAmount&pageEntity.index=1"/>"
                    title="销  量">销 量</a></li>
            <li id="time"><a
                    href="<c:url value="/product/list/${project.id}?sort=desc:product.createDateTime&pageEntity.index=1"/>"
                    title="新  品">新 品</a></li>
        </ul>
    </div>
    <!-- //End--shop-sort-->
