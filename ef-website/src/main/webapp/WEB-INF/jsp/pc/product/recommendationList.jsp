...
<head>
    ...
    <meta name="keywords"
          content="${productModel.product.project.name},${master.name},${productModel.product.name},${productModel.product.subName}, ${productModel.name}"/>
    <meta name="description"
          content="${productModel.name},${productModel.product.name},${productModel.product.subName},${productModel.product.project.description}"/>
...
<div class="hd explosion">
    <div class="details wh">
        <div class="img"><a href="<c:url value="/product/productModel/${productModel.id}"/>" target="_blank"
                            title=""><img class="imgfilter"
                                          src="http://pro.efeiyi.com/${productModel.productModel_url}@!product-hot-picture"/></a>
        </div>
        ...
        <div class="info">
            ...
            <div class="p-img">
                ...
                <c:if test="${not empty productModel.product.master.id}">
                    <a href="http://${productModel.product.master.name}.efeiyi.com" target="_blank" title=""><img
                            class="imgfilter"
                            src="http://tenant.efeiyi.com/${productModel.product.master.favicon}@!ef-home-master"
                            target="_blank" alt=""/></a>
                    <div class="des-txt">

                        <p><a href="http://${productModel.product.master.name}.efeiyi.com" target="_blank"
                              title="">${productModel.product.master.fullName}</a></p>
                        <p>[${productModel.product.master.getMainProjectName().getProject().getName()}]</p>
            ...
            <c:if test="${productModel.amount<=0}">
                <div class="item p-btn active">
                    <a class="btn btn-append" href="<c:url value="/product/productModel/${productModel.id}"/>"
                       title="查看详情"><i class="icon"></i>查看详情</a>
                </div>
            </c:if>
            <c:if test="${productModel.amount>0}">
                <div class="item p-btn">
                    <a class="cart" href="#"  onclick="addCart('${productModel.id}')" title="加入购物车"><i class="icon"></i>加入购物车</a>
...
<script>
    //跟踪加入购物车事件
    function addCart(o) {
        window.location.href = "<c:url value="/cart/addProduct.do?id="/>" + o + "&amount=" + 1 + "&redirect=/product/productModel/" + o;
    }
</script>
