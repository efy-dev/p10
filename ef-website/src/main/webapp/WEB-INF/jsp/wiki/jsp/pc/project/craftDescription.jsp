...
<div class="craft-particulars2016">
    <div class="craft-crumbs">
        <div class="particulars">
            <div class="crumbs"><a href="http://minglu.efeiyi.com/">首页</a><em class="">></em><a
                    href="http://minglu.efeiyi.com/" class=""><ming800:status name="type" dataType="Project.type"
                                                                              checkedValue="${artistry.getType()}"
                                                                              type="normal"/></a><a href=""
                                                                                                    class="present"><em
                    class="present">></em>${artistry.project.name}</a></div>
        </div>
    </div>
    <div class="user">
        <div class="skill">
            <div class="page">
                <div class="txt-page">
                    ...
                    <p class="rank">技艺级别：<em><ming800:status name="level" dataType="Project.level"
                                                             checkedValue="${artistry.getLevel()}" type="normal"/></em>
                    </p>
                    <c:if test="${not empty artistry.project.addressDistrict}">
                        <p class="area">
                            所属地区：<em>${artistry.project.addressDistrict.addressCity.addressProvince.name}${artistry.project.addressDistrict.addressCity.name}${artistry.project.addressDistrict.name}</em>
                        </p>
                    </c:if>
                </div>
                <div class="pic-page">
                    <c:if test="${not empty masterProjectList&&fn:length(masterProjectList)>0}">
                        <div class="user-pic">
                            ...
                            <a href="http://${masterProjectList.get(0).getMaster().name}.efeiyi.com/">
                                <img src="<c:url value="http://tenant.efeiyi.com/${masterProjectList.get(0).getMaster().getFavicon()}@!wiki-pc-master-picture"/>"
                                     alt="">
                                <div class="user-txt">
                                    <p class="name">${masterProjectList.get(0).getMaster().getFullName()}</p>
                                    <p class="craft">${masterProjectList.get(0).getProject().getName()}</p>
                                    <p class="rank"><ming800:status name="level" dataType="Project.level"
                                                                    checkedValue="${masterProjectList.get(0).getProject().getLevel()}"
                                                                    type="normal"/>传承人</p>
                                </div>
                            </a>
                        </div>
