...
<header id="header" class="am-header custom-header index-header newheader">
    <div class="logo"><a class="icon" style="background:none;width:110px;" href="" title="e飞蚁"><img
    ...
    <div class="list bd">
        ...
        <ul class="ul-col-2" id="artistryId">
            <c:forEach items="${artistryList}" var="artistry" begin="0" end="7">
                <li>
                    <a href="<c:url value="/minglu/project/${artistry.id}"/>" title="">
                        <img src="http://wiki-oss.efeiyi.com/${artistry.getMainPicture().pictureUrl}@!wiki-project-picture"
                             alt="">
                        <span class="txt">
                        <p>${artistry.name}</p>
                        <p><ming800:status name="level" dataType="Project.level" checkedValue="${artistry.getLevel()}"
                                           type="normal"/>非遗项目</p>
                    </span>
                    </a>
                </li>
...
<script type="text/javascript">
    function htmlGenerater(it /**/) {
        ...
        for (var i = 0; i < it.length; i++) {
            out += '<li> <a href="<c:url value="/minglu/project/"/>' + (it[i].id) + '"><img src="http://wiki-oss.efeiyi.com/' + (it[i].pictureUrl) + '@!wiki-project-picture"></a><span class="txt"><p>';
            out += it[i].name;
            out += '</p><p>';
            switch (it[i].level) {
                case 1:
    ...
    function moreArtistry() {
        var url = "<c:url value='/minglu/artistrySearch.do/'/>";
        getArtistryList(url);
    }
