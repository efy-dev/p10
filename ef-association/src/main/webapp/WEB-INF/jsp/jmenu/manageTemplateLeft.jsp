    ...
    <ul class="am-list admin-sidebar-list">
      <c:forEach items="${jnode.children}" var="childJnode">
              ...
              <c:forEach items="${childJnode.children}" var="childchildJnode">
                <li>
                  <a href="<c:url value='${childchildJnode.url}'/> "
                     class="am-cf ${childchildJnode.jnodeMatch('ass-active',currentJnode!=null?currentJnode:"")}">${childchildJnode.text_zh_CN}</a>
                </li>
              </c:forEach>
            </ul>
          </li>
        ...
        <c:if test="${empty childJnode.children}">
          <li><a class="${childJnode.jnodeMatch('ass-active',currentJnode!=null?currentJnode:"")}" href="<c:url value='${childJnode.url}'/>"> ${childJnode.text_zh_CN}</a></li>
        </c:if>
      </c:forEach>
    </ul>
