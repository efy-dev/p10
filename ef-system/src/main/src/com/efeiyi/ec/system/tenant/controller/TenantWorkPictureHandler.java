 ...
 */
public class TenantWorkPictureHandler implements MultipartHandler {
    ...
    public ModelMap handleMultipart(Do tempDo, ModelMap modelMap, HttpServletRequest request, MultipartRequest multipartRequest) throws Exception {
        ...
        String tenantWorkId = request.getParameter("tenantWorkId");
        String tenantId = request.getParameter("tenant.id");
        MultipartFile multipartFile = multipartRequest.getFile("pictrue");
        String url = "work/" + tenantId + "/" +tenantWorkId+"/"+ multipartFile.getOriginalFilename();
        ...
        paramMap.put("product.id", tenantWorkId);
