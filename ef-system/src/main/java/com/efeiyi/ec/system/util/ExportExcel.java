...
//            File downloadDir = new File("D://CouponBatchDownload");
//            File file = null;
//            if(!downloadDir.exists()){
//                downloadDir.mkdir();
//            }
//            File downloadDirTemp = new File("/com/efeiyi/ec/system/download");
//            if(!downloadDirTemp.exists()){
//                downloadDirTemp.mkdir();
//            }
    ...
    public final static Object exportExcel(Object[] titles, List<Object[]> contents, OutputStream os) {
        try {
            ...
            for (int i = 0; i < titles.length; i++) {
                sheet.addCell(new Label(i, 0, (String) titles[i], wcf_center));
            }
            /** ***************以下是EXCEL正文数据********************* */
            for (int i = 1; i <= contents.size(); i++) {
                for (int j = 0; j < contents.get(i - 1).length; j++) {
                    sheet.addCell(new Label(j, i, (String) contents.get(i - 1)[j], wcf_left));
                }
            }
            /** **********将以上缓存中的内容写到EXCEL文件中******** */
            workbook.write();
