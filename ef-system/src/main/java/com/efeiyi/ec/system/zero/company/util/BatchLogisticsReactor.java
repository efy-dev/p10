...
//                    purchaseOrderProduct.getPurchaseOrder().setOrderStatus(PurchaseOrder.ORDER_STATUS_FAILED);//发货失败
                    purchaseOrderProduct.getPurchaseOrder().setMessage((" DEPPON：" + dateFormat.format(new Date()) + " 原因：" + result + " " + purchaseOrderProduct.getPurchaseOrder().getMessage()).substring(0,1024));
                    //发货失败，自动修改订单号
