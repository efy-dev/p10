...
//        query.executeUpdate();
    ...
    @Override
    public List getResult(String startTime,String endTime,String selectTenant) {
        String sql = "SELECT po.serial as 订单号,case po.payway when 1 then '支付宝' when 2 then '银行卡' when 3 then '微信支付' when 4 then '优惠券' end as 支付方式,po.total as 订单总价,case po.order_status when 1 then '等待付款' WHEN  3 THEN  '等待成团'  when 5 then '已付款' WHEN  51 THEN '已确认发货中' WHEN 6 then '待收礼' when 7 then '已发货' when 9 then '未评价' when 13 then '已完成' when 15 then '已退款' when 17 then '已取消' WHEN 19 THEN '待扫运单' end as 订单状态,po.original_price as 订单原价,po.purchase_order_address as 收货人地址,po.receiver_name as 收货人姓名,po.receiver_phone as 收货人联系方式,po.create_datetime as 下单时间, " +
                ...
                "       t.name as 店铺,case po.order_type when 1 then '普通类型' when 2 then '秒杀类型' when 3 then '礼品卷类型' when 4 then '团购类型' when 5 then '企业礼品卡类型' end as 订单类型 ,pm.product_model_url as '商品链接' " +
        ...
        if (!"".equals(startTime) && startTime != null){
            sql+=" and po.create_datetime>='" +startTime+"'";
        }
        if(!"".equals(startTime) && startTime != null){
            sql+=" and po.create_datetime<='" +endTime+"'";
        }
        if(!"".equals(selectTenant) && selectTenant != null){
            sql+=" and t.id='"+selectTenant+"'";
        }
        sql+= " order by po.create_datetime desc ";
