        ...
        <c:forEach items="${masters}" var="obj">
            <div class="inheritor">
                <div class="inheritor-text">
                    ...
                    <c:if test="${obj.projectName != null}">
                        <p class="itor-text-2">${obj.projectName}</p>
                    </c:if>
                    <p class="itor-text-3"><ming800:status name='level' dataType='Master.level'
                                                           checkedValue='${obj.level}' type='normal'/>非遗传承人</p>
                    <p class="itor-text-4">${obj.brief}</p>
                    <div class="gz-fd-img"><a
                            href="<c:url value='/masterMessage/forwardMasterDetails.do?masterId=${obj.id}'/>"><img
                            src="http://tenant.oss-cn-beijing.aliyuncs.com/${obj.favicon}"></a></div>
                </div>
            </div>
        </c:forEach>
    </div>
