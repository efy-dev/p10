    ...
    <div class="list-f-select">
        <div class="f-select-group">
            <div class="select-head">
                <span>分类：</span>
                <strong><a onclick="getData(this,'<c:url
                        value="/masterCategory/getClassifyData.do?qm=plistMaster_all&pageEntity.size=6&pageEntity.index="/>')"
                           about="1" class="active">全部</a></strong>
            </div>
            <ul class="select-list" id="projectClassify">
                <c:if test="${!empty categoryList}">
                    <c:forEach items="${categoryList}" var="cate" varStatus="cat">
                        <li><a about="0" onclick="getData(this,'<c:url
                                value="/masterCategory/getClassifyData.do?qm=plistMasterProject_default&conditions=category.id:${cate.id};&pageEntity.size=6&pageEntity.index="/>');">${cate.name}</a>
                        </li>
                    </c:forEach>
                </c:if>
            </ul>
        ...
        <%--<div class="f-select-group cl-act">--%>
            <%--<div class="select-head">--%>
                <%--<span>地区：</span>--%>
                <%--<strong><a about="0" onclick="getData(this,'<c:url--%>
                        <%--value="/masterCategory/getClassifyData.do?qm=plistMaster_all&pageEntity.size=6&pageEntity.index="/>')">全部</a></strong>--%>
            <%--</div>--%>
            <%--<ul class="select-list" id="cityClassify">--%>
                <%--<c:if test="${!empty cityList}">--%>
                    <%--<c:forEach items="${cityList}" var="city">--%>
                        <%--<li><a about="0" onclick="getData(this,'<c:url--%>
                                <%--value='/masterCategory/getClassifyData.do?qm=plistMaster_byCity&conditions=originProvince.id:${city.id};&pageEntity.size=6&pageEntity.index='/>')">${city.name}</a>--%>
                        <%--</li>--%>
                    <%--</c:forEach>--%>
                <%--</c:if>--%>
            <%--</ul>--%>
...
//          pubu.empty();
                    for (var i in data) {
                        ...
                        switch (data[i].level) {
                            case "1":
                                levelName = "国家级" + data[i].projectName + "代表性传承人";
                                break;
                            case "2":
                                levelName = "省级" + data[i].projectName + "代表性传承人";
                                break;
                            case "3":
                                levelName = "市级" + data[i].projectName + "代表性传承人";
                                break;
                            default:
                                levelName = "县级" + data[i].projectName + "代表性传承人";
                        }
